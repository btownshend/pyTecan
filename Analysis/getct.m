...
for i=1:length(sel)
  ...
  for k=1:length(o)
    ...
    map(nmk)=vols(k);
    if ~any(strcmp(nmk,{'SSD','MPosRT','MNegRT'})) && ~strncmp(upper(nmk),'WATER',5) && ~strncmp(nmk,'MQ',2)  && ~strncmp(nmk,'MLig',4) && ~strncmp(nmk,'MSt',3) && ~strcmp(nmk,'Dynabeads') && ~strcmp(nmk,'BeadBuffer')
      if nmk(1)=='M'
