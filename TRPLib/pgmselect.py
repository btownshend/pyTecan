...
class PGMSelect(TRP):
    ...
    def oneround(self,q,input,prefixOut,analytic,prefixIn,keepCleaved,t7vol,rtvol,pcrdil,cycles,pcrvol):
        if keepCleaved:
        ...
        if analytic:
            ...
            #self.runRTInPlace(src=rxs,vol=rtvol,heatInactivate=True,hiTemp=hiTemp,dur=rtDur,incTemp=50)    # Heat inactivate also allows splint to fold
            rxs=self.runRT(src=rxs,vol=rtvol,srcdil=rtDil,heatInactivate=True,hiTemp=hiTemp,dur=rtDur,incTemp=50,stop=[reagents.getsample(s) for s in stop])    # Heat inactivate also allows splint to fold
            print "RT volume= ",[x.volume for x in rxs]
        needDil /= rtDil
        rtPostDil=5 if analytic else 1
        if rtPostDil!=1:
        ...
        if keepCleaved:
            ...
            extpostdil=4 if analytic else 2 # max(1.0,round(100/rxs[0].volume,1))
            if extpostdil>1:
        ...
        if self.dopcr:
            ...
            pcrpostdil=2 if analytic else 1
            if pcrpostdil>1:
